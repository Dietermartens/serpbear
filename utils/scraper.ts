...
const getScraperApiError = (res: Record<string, unknown>, scraperType: string): string => {
	if (typeof res.status === "number" && res.status !== 0) {
		return String(res.error || res.detail || `API error (status ${res.status})`);
	}
	return String(res.error || res.detail || `Empty response from ${scraperType || "scraper"}`);
};
...
export const getScraperClient = (
	...
	if (settings && settings.scraper_type === "proxy" && settings.proxy) {
		...
		client = axiosClient.get(
			`https://www.google.com/search?num=${p.num}&start=${p.start}&q=${encodeURI(keyword.keyword)}`,
		);
	} else {
		const method = scraper?.scrapeMethod || "GET";
		const fetchOptions: RequestInit = { method, headers };
		if (method === "POST" && scraper?.requestBody) {
			fetchOptions.body = JSON.stringify(scraper.requestBody(keyword, settings, countries, pagination));
		}
		client = fetch(apiURL, fetchOptions);
	}
...
const scrapeSinglePage = async (
	...
	if (!scraperClient) {
		return { results: [], error: "No scraper client available" };
	}
	try {
		const res =
		...
		if (typeof res.status === "number" && res.status !== 0) {
			return { results: [], error: getScraperApiError(res, scraperType) };
		}
		const scraperResult =
			scraperObj?.resultObjectKey && res[scraperObj.resultObjectKey] ? res[scraperObj.resultObjectKey] : "";
...
const resolveStrategy = (
	...
	const smartFullFallback: boolean =
		domainSettings?.scrape_smart_full_fallback || settings.scrape_smart_full_fallback || false;
...
export const scrapeKeywordWithStrategy = async (
	...
	if (strategy === "smart" && smartFullFallback) {
		const serpCheck =
			allScrapedResults.length > 0
				? getSerp(keyword.domain, allScrapedResults, subdomainMatching)
				: { position: 0, url: "" };
		if (serpCheck.position === 0) {
			const alreadyScraped = new Set(pagesToScrape);
			const remainingPages = Array.from({ length: TOTAL_PAGES }, (_, i) => i + 1).filter(
				(p) => !alreadyScraped.has(p),
			);
	...
	console.log(
		...
		finalSerp.position,
		finalSerp.url,
		`(strategy: ${strategy}), Skipped ${skippedCount}`,
	);
	return {
		ID: keyword.ID,
		keyword: keyword.keyword,
	...
	try {
		const res =
			scraperType === "proxy" && settings.proxy
				...
				: await scraperClient.then((result: any) => result.json());
		if (typeof res.status === "number" && res.status !== 0) {
			scraperError = getScraperApiError(res, scraperType);
		...
		if (res && scrapeResult) {
			const extracted = scraperObj?.serpExtractor
				? scraperObj.serpExtractor(scrapeResult)
				: extractScrapedResult(scrapeResult, keyword.device);
			await writeFile("result.txt", JSON.stringify(scrapeResult), { encoding: "utf-8" }).catch((err) => {
				console.log(err);
			});
...
export const getSerp = (domainURL: string, result: SearchResult[], subdomainMatching?: string): SERPObject => {
	...
	const foundItem = result.find((item) => {
		...
		if (matchAllSubdomains && (itemHost === baseDomain || itemHost.endsWith(`.${baseDomain}`))) {
			return true;
		}
		return false;
	});
